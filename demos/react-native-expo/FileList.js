...
import getFileTypeIcon from '@Burp/dashboard/lib/utils/getFileTypeIcon.js'
...
function BurpDashboardFileIcon ({ type }) {
...
export default function FileList ({ Burp }) {
  const BurpFiles = Burp.store.state.files
  const BurpFilesArray = Object.keys(BurpFiles).map((id) => BurpFiles[id])
          ...
          return (
            <View style={styles.item}>
              {item.type === 'image' ? (
                ...
                <BurpDashboardFileIcon type={item.type} />
              )}
