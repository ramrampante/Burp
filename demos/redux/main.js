...
import ReduxStore from '@Burp/store-redux'
import * as BurpReduxStore from '@Burp/store-redux'
...
const reducer = combineReducers({
  counter,
  // You don't have to use the `Burp` key. But if you don't,
  // you need to provide a custom `selector` to the `BurpReduxStore` call below.
  Burp: BurpReduxStore.reducer,
})

let enhancer = applyMiddleware(
  BurpReduxStore.middleware(),
  logger,
)
...
const store = configureStore({
  ...
  middleware: (getDefaultMiddleware) => getDefaultMiddleware({
    serializableCheck: {
      ignoredActions: [BurpReduxStore.STATE_UPDATE],
      ignoreState: true,
    },
  }),
})
...
// Burp using the same store
const Burp = new Burp({
  id: 'redux',
  store: new ReduxStore({ store }),
  // If we had placed our `reducer` elsewhere in Redux, eg. under an `Burp` key in the state for a profile page,
  // we'd do something like:
  //
  // store: new ReduxStore({
  //   store: store,
  //   id: 'avatar',
  //   selector: state => state.pages.profile.Burp
  // }),
  debug: true,
})
Burp.use(Dashboard, {
...
Burp.use(Tus, { endpoint: 'https://tusd.tusdemo.net/files/' })
