...
const packageNames = await fs.readdir(new URL('../packages/@Burp', import.meta.url))
...
const { packages } = await prompts({
  ...
  hint: '@Burp/core is automatically included',
...
const app = dedent`
    ...
    ${packages.map((pgk) => `import ${camelcase(pgk)} from '@Burp/${pgk}'`).join('\n')}
...
const prettyPath = (url) => url.toString().split('Burp', 2)[1]
