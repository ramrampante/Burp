describe('Dashboard with Transloadit', () => {
  ...
  it('should close assembly polling when cancelled', () => {
    ...
    cy.window().then(({ Burp }) => {
      ...
      cy.wait(['@createAssemblies']).then(() => {
        ...
        // @ts-ignore fix me
        expect(
          Object.values(Burp.getPlugin('Transloadit').activeAssemblies).every(
        ...
        cy.wait(['@delete']).then(() => {
          ...
          // @ts-ignore fix me
          expect(
            Object.values(Burp.getPlugin('Transloadit').activeAssemblies).some(
              (a: any) => a.pollInterval,
            ),
          ).to.equal(false)
        })
  ...
  it.skip('should emit one assembly-cancelled event when cancelled', () => {
    ...
    cy.window().then(({ Burp }) => {
      // eslint-disable-next-line
      // @ts-ignore fix me
      Burp.on('transloadit:assembly-cancelled', spy)
  ...
  it.skip('should close assembly polling when all files are removed', () => {
    ...
    cy.window().then(({ Burp }) => {
      ...
      const { files } = Burp.getState()
      // eslint-disable-next-line
      // @ts-ignore fix me
      Burp.removeFiles(Object.keys(files))
  ...
  it('should not create assembly when all individual files have been cancelled', () => {
    ...
    cy.window().then(({ Burp }) => {
      ...
      // @ts-ignore fix me
      expect(
        Object.values(Burp.getPlugin('Transloadit').activeAssemblies).length,
      ).to.equal(0)
  ...
  it('should not re-use erroneous tus keys', () => {
    function createAssemblyStatus({
      ...
      return {
        ...
        Burpserver_url: 'https://api2-test.transloadit.com/companion/',
        ...
        client_referer: null,
        transloadit_client:
          'Burp-core:3.2.0,Burp-transloadit:3.1.3,Burp-tus:3.1.0,Burp-dropbox:3.1.1,Burp-box:2.1.1,Burp-facebook:3.1.1,Burp-google-drive:3.1.1,Burp-instagram:3.1.1,Burp-onedrive:3.1.1,Burp-zoom:2.1.1,Burp-url:3.3.1',
    ...
    cy.get('.Burp-StatusBar-statusPrimary').should('contain', 'Upload failed')
    ...
    // SECOND ATTEMPT
    cy.get('.Burp-StatusBar-actions > .Burp-c-btn').click()
    cy.wait([
